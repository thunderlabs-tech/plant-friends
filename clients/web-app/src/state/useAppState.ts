...
type ReducerActionObjects<State, ActionFns extends ActionFnType<State>> = {
  [Key in keyof ActionFns]: { type: Key; params: Parameters<OmitFirstArg<ActionFns[Key]>> };
}[keyof ActionFns];

type RemoveReturnType<T extends (...args: any) => any> = (...a: Parameters<T>) => void;

type ExternalActionFn<State, ActionFn extends (currentState: State, ...restArgs: any) => any> = RemoveReturnType<
